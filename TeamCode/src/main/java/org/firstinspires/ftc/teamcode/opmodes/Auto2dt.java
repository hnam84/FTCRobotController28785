...
@Autonomous(name = "Auto2dt", group = "Linear OpMode")
public class Auto2dt extends BaseOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        Action autoRoutine = drive.actionBuilder(initialPose)
                .strafeToLinearHeading(new Vector2d(-17, 15), Math.toRadians(200))
                // --- BẮN 3 QUẢ ĐẦU (DỰA TRÊN TAG) ---
                .strafeToLinearHeading(new Vector2d(-17, 15), Math.toRadians(-226))
                .stopAndAdd(setShooterAction(Constants.FAR_SHOOTER_SPEED))
                .waitSeconds(0.5)
                .stopAndAdd(executeSmartShooting(shootingOrder))
                .stopAndAdd(setShooterAction(0))

                // --- ĐI HÚT BÓNG 1 VÀ QUAY LẠI BẮN ---
                .strafeToLinearHeading(new Vector2d(-11, 30), Math.toRadians(-270))
                .afterDisp(0, setIntakeAction(Constants.INTAKE_SPEED)) // Bật intake khi đang di chuyển
                .strafeToLinearHeading(new Vector2d(-11, 48), Math.toRadians(-270))
                .waitSeconds(0.5)
                .stopAndAdd(setIntakeAction(0))

                .strafeToLinearHeading(new Vector2d(-17, 15), Math.toRadians(-226))
                .stopAndAdd(setShooterAction(Constants.FAR_SHOOTER_SPEED))
                .stopAndAdd(executeSmartShooting(new String[]{"GREEN", "PURPLE1", "PURPLE2"}))
                .stopAndAdd(setShooterAction(0))

                // --- ĐI HÚT BÓNG 2 VÀ QUAY LẠI BẮN ---
                .strafeToLinearHeading(new Vector2d(12, 30), Math.toRadians(-270))
                .afterDisp(0, setIntakeAction(Constants.INTAKE_SPEED))
                .strafeToLinearHeading(new Vector2d(12, 48), Math.toRadians(-270))
                .waitSeconds(0.5)
                .stopAndAdd(setIntakeAction(0))
                .strafeToLinearHeading(new Vector2d(-2, 56), Math.toRadians(-270))

                .strafeToLinearHeading(new Vector2d(-17, 15), Math.toRadians(-226))
                .stopAndAdd(setShooterAction(Constants.FAR_SHOOTER_SPEED))
                .stopAndAdd(executeSmartShooting(new String[]{"GREEN", "PURPLE1", "PURPLE2"}))
                .stopAndAdd(setShooterAction(0))
                .strafeToLinearHeading(new Vector2d(0, 20), Math.toRadians(-270))
                .build();
